...
const QueryGenerator = {
  ...
  jsonPathExtractionQuery(column, path) {
    ...
    const quotedColumn = `"${column.join('.').replace('.', '"."')}"`
    ...
    if (smth instanceof Utils.Json) {
      ...
      if (smth.conditions) {
        const conditions = _.map(this.parseConditionObject(smth.conditions), condition =>
          `${this.jsonPathExtractionQuery([(smth.tableAlias || '').replace(/[\W]/g, ''), _.first(condition.path)], _.tail(condition.path))} ${smth.comparator} '${condition.value}'`
        );
        ...
        if (this._checkValidJsonStatement(smth.path)) {
          ...
          str = this.jsonPathExtractionQuery([(smth.tableAlias || '').replace(/[\W]/g, ''), column], paths);
        }

        if (smth.value) {
          str += util.format(' %s %s', smth.comparator, this.escape(smth.value));
        }
